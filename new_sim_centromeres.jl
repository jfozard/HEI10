...
# Generate random RI positions
    ...
    rho::Float64 = args["c_dsb_ratio"]
    c_start::Float64 = args["c_start"]
    c_end::Float64 = args["c_end"]

    x0 = rand(mt, N)

    local c_mid::Float64
    if args["c_mid"]==nothing
        c_mid = 0.5*(c_start+c_end)
    else
        c_mid = args["c_mid"]
    end

    # Generate RI piecewise0-linear density profile and sample N points from it
    h = linear_density_inverse([0.0, c_start, c_mid, c_end, 1.0], [1.0, 1.0, rho, 1.0, 1.0])
    x1 = map(h, x0)
    x::Array{Float64,1} = L * x1
    ...
    C0_t = 1.0 
    C0_t_L::Float64 = Float64(args["t_C0_ratio_L"]) # ($f_e$) at LH end
    C0_t_R::Float64 = Float64(args["t_C0_ratio_R"]) # ($f_e$) at RH end
    ...
    if !args["t_exp"]
        ...
        for i in 1:N
            ...
            if p < t_L
               r = p/t_L
                C0[i] *= C0_t*(r) + C0_t_L*(1-r)
            elseif p> 1-t_L
               r = (1-p)/t_L
               C0[i] *= C0_t*(r) + C0_t_R*(1-r)
       	     end
	 end
    else
    ...
    @add_arg_table! s begin
       ...
       "--Ls"
        ...
        "--t_C0_ratio_L"
            ...
            help="initial hei10 ratio for RIs in telomeres" # (f_e) Increased RI loading at LH SC telomere
        "--t_C0_ratio_R"
            arg_type=Float64
            default=2.0 # (no units)
            help="initial hei10 ratio for RIs in telomeres" # (f_e) Increased RI loading at RH SC telomere
	"--t_exp"
        ...
        "--c_dsb_ratio"
            arg_type=Float64
            default=0.5 # (no units)
            help="centromere dsb density"
        "--c_start"
            arg_type=Float64
            default=0.25 # (no units)
            help="centrometre start pos"

        "--c_end"
             arg_type=Float64
             default=0.35 # (no units)
             help="centrometre end pos"
        "--c_mid"
             arg_type=Float64
             help="centrometre middle"
